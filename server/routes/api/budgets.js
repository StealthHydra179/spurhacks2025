...
router.get("/:id", authenticateToken, async function (req, res) {
  try {
    ...
    if (budget.user_id !== userId) {
      logger.warn(`${TAG} User ${userId} attempted to access budget ${budgetId} belonging to user ${budget.user_id}`);
...
router.post("/", authenticateToken, async function (req, res) {
  try {
    ...
    const categories = { housing, food, transportation, health, personal, entertainment, financial, gifts };
    ...
    if (categoryTotal > overall) {
      logger.warn(`${TAG} Category total (${categoryTotal}) exceeds overall budget (${overall})`);
      return res.status(400).json({
        status: "error",
...
router.put("/:id", authenticateToken, async function (req, res) {
  try {
    ...
    if (existingBudget.user_id !== userId) {
      logger.warn(`${TAG} User ${userId} attempted to update budget ${budgetId} belonging to user ${existingBudget.user_id}`);
      return res.status(403).json({
        status: "error",
        message: "Access denied"
      });
...
router.patch("/:id", authenticateToken, async function (req, res) {
  try {
    ...
    if (existingBudget.user_id !== userId) {
      logger.warn(`${TAG} User ${userId} attempted to patch budget ${budgetId} belonging to user ${existingBudget.user_id}`);
      return res.status(403).json({
        status: "error",
        message: "Access denied"
      });
    ...
    for (const [field, value] of Object.entries(updates)) {
      ...
      if (field === 'overall' && (value === undefined || value === null || value <= 0)) {
    ...
    const newOverall = updates.overall !== undefined ? updates.overall : existingBudget.overall;
    const categories = ['housing', 'food', 'transportation', 'health', 'personal', 'entertainment', 'financial', 'gifts'];
    const categoryTotal = categories.reduce((sum, category) => {
      const value = updates[category] !== undefined ? updates[category] : existingBudget[category];
      return sum + (value || 0);
    }, 0);
    
    if (categoryTotal > newOverall) {
      logger.warn(`${TAG} Category total (${categoryTotal}) would exceed overall budget (${newOverall})`);
      return res.status(400).json({
        status: "error",
        message: "Sum of category budgets cannot exceed overall budget"
      });
...
router.delete("/:id", authenticateToken, async function (req, res) {
  try {
    ...
    if (existingBudget.user_id !== userId) {
      logger.warn(`${TAG} User ${userId} attempted to delete budget ${budgetId} belonging to user ${existingBudget.user_id}`);
      return res.status(403).json({
        status: "error",
        message: "Access denied"
      });
