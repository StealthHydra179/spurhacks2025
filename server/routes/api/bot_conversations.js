...
router.post("/create", authenticateToken, async function (req, res) {
  try {
    ...
    const newConversation = await botConversationsDb.createConversation(
      userID,
      summary
    );

    logger.info(
      `${TAG}: Created new conversation with ID ${newConversation.id} for user ${userID}`
    );
...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      logger.info(
        `${TAG}: Added message ${newMessage.message_number} to conversation ${conversationId} by ${sender}`
      );
      ...
      logger.error(
        `${TAG}: Error adding message to conversation: ${error.message}`
      );
      res.status(500).json({
        message: "Internal server error",
        status: "error",
...
router.get(
  ...
  async function (req, res) {
    try {
      const userID = req.user.userID;
      const conversations = await botConversationsDb.getUserConversations(
        userID
      );
      ...
      logger.error(
        `${TAG}: Error fetching user conversations: ${error.message}`
      );
      res.status(500).json({
        message: "Internal server error",
        status: "error",
...
router.post("/ask-capy", authenticateToken, async (req, res) => {
  try {
    ...
    await botConversationsDb.addMessageToConversation(
      newConversation.id,
      question,
      "user"
    );
    ...
    logger.info(
      `${TAG}: Created new conversation ${newConversation.id} for user ${userID} with question: ${question}`
    );
    ...
    try {
      ...
      logger.info(
        `${TAG}: Transaction Data ${JSON.stringify(transactionData)}`
      );
      ...
      logger.info(
        `${TAG}: Added AI response to conversation ${newConversation.id}`
      );
    } catch (aiError) {
      logger.error(
        `${TAG}: Error generating AI response for conversation ${newConversation.id}: ${aiError.message}`
      );
