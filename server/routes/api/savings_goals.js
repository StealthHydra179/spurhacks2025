...
router.get("/:id", authenticateToken, async function (req, res) {
  try {
    ...
    if (savingsGoal.user_id !== userId) {
      logger.warn(
        `${TAG} User ${userId} attempted to access savings goal ${goalId} belonging to user ${savingsGoal.user_id}`
      );
...
router.post("/", authenticateToken, async function (req, res) {
  try {
    ...
    if (current_amount < 0) {
      logger.warn(
        `${TAG} Invalid current_amount for savings goal: ${current_amount}`
      );
      return res.status(400).json({
        status: "error",
...
router.patch("/:id", authenticateToken, async function (req, res) {
  try {
    ...
    if (updateData.amount !== undefined && updateData.amount <= 0) {
      logger.warn(
        `${TAG} Invalid amount for savings goal: ${updateData.amount}`
      );
      return res.status(400).json({
        status: "error",
        message: "Amount must be greater than 0",
    ...
    if (
      ...
      logger.warn(
        `${TAG} Invalid current_amount for savings goal: ${updateData.current_amount}`
      );
      return res.status(400).json({
        status: "error",
        message: "Current amount cannot be negative",
    ...
    logger.info(
      `${TAG} Partially updating savings goal ${goalId} for user: ${userId}`
    );
...
router.get("/stats/summary", authenticateToken, async function (req, res) {
  try {
    ...
    savingsGoals.forEach((goal) => {
      ...
      if (
        goal.priority &&
        stats.goals_by_priority.hasOwnProperty(goal.priority)
      ) {
      ...
      // Count by category
      if (goal.category) {
        stats.goals_by_category[goal.category] =
          (stats.goals_by_category[goal.category] || 0) + 1;
      }
    });
    ...
    logger.error(
      `${TAG} Error fetching savings goals statistics: ${error.message}`
    );
    res.status(500).json({
      status: "error",
      message: "Internal Server Error",
